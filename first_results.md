...
## Every table now generated

The audit found all seven errors in the five tables still typed by hand, so those are now built by
`scripts/make_paper_tables.py` from the same results files the prose is checked against. All 17
tables in the paper are `\input` and `main.tex` contains no `tabular` of its own. Row labels and
the reproduction ledger's "specified in the paper" column stay as literals in the generator; they
are prose, not measurements.

Three of the five regenerated byte-identically against the corrected hand versions, which is the
evidence the generator reads the right sources. The other two differed in three places, all of
them the hand version being wrong:

- **Rounding.** Two cells sat exactly on a half — Family B's 3σ Gaussian decoy rate at 12.5% and
  the second lens-light seed set's multipole rate at 32.5%. Python rounds halves to even, so a
  naive generator prints 12 and 32 where the results file says 12.5 and 32.5. The generator rounds
  half away from zero, which is what a reader checking a table expects, giving 13 and 33.
- **Cost per lens.** The joint-versus-frozen table read 1 s and 30 s. Neither run's manifest gives
  30 s under any interpretation, because the two runs used different worker counts and the frozen
  run never recorded its own. Both runs store per-lens timers, which settle it independently of
  parallelism: median 0.7 s frozen, 35 s for the joint scan. The 30 s was the *uncontrolled* joint
  run's mean (31.3 s), not the basin-controlled run the rest of the column reports.
- **The basin control's cost.** The appendix said the control "doubles the 30 s per-lens cost". The
  timers give 35 s for the scan and 42.5 s with the control, a 21% increase, not a doubling.
  Reworded to the measured figures.
